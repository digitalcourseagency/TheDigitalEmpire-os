  ...
  return (
    <div style={{maxWidth:1100,animation:"slideUp 0.4s ease"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:28}}>
        ...
        <button className="btn btn-primary" onClick={()=>{setForm({urgency:"Medium",status:"Idea"});setShowModal(true);}}>
      ...
      {loading ? (
        ...
        <div style={{textAlign:"center",padding:"64px 0"}}>
          ...
          <button className="btn btn-primary" onClick={()=>{setForm({urgency:"Medium",status:"Idea"});setShowModal(true);}}>Capture First Idea</button>
        </div>
      ) : (
        <div>
          {["High","Medium","Low"].map(urgency => {
            ...
            return (
              <div key={urgency} style={{marginBottom:28}}>
                <div style={{fontSize:11,letterSpacing:"0.1em",textTransform:"uppercase",color:URGENCY_COLOR[urgency],fontWeight:500,marginBottom:12}}>
                  {urgency} Urgency <span style={{color:"#B8B0A5"}}>({items.length})</span>
                </div>
        ...
        <div className="modal-overlay" onClick={e=>e.target===e.currentTarget&&setShowModal(false)}>
          <div className="modal">
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
              ...
              <button onClick={()=>setShowModal(false)} style={{background:"none",border:"none",cursor:"pointer",fontSize:20,color:"#9A9188"}}>&times;</button>
            </div>
            <div style={{display:"grid",gap:12}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
                <select className="input" value={form.brand_id||""} onChange={e=>setForm({...form,brand_id:e.target.value})}>
                  <option value="">&mdash; Brand &mdash;</option>
                  {BRANDS.map(b=><option key={b.id} value={b.id}>{b.name}</option>)}
                </select>
                <select className="input" value={form.urgency||"Medium"} onChange={e=>setForm({...form,urgency:e.target.value})}>
